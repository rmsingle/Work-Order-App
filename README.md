...
| Fast Capture | **Add photo** is a compact gold button on each jobs-list card (about the size of the title, not a full-width bar) and on the job detail page: navy header and gold button under the title. The sheet takes a note, then camera or library (web falls back to a file picker). Upload saves the note on `job_photos.caption` |
